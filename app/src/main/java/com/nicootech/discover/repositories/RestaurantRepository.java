...
import static com.nicootech.discover.util.Constants.BASE_URL;
...
public class RestaurantRepository {
    ...
    private LiveData restaurantList;
        ...
        restaurantWebService.getRestaurantDetail(id).enqueue(new Callback<RestaurantV2>() {
            ...
            public void onResponse(Call<RestaurantV2> call, Response<RestaurantV2> response) {
                if (response.isSuccessful()) {
                    Log.d(TAG, "get restaurant detail success: " + response.body().toString());
                    restaurantLiveData.setValue(response.body());
                } else {
                    Log.d(TAG, "get restaurant detail fail: " + response.body().toString());
                }
            }

            @Override
            public void onFailure(Call<RestaurantV2> call, Throwable t) {
                Log.d(TAG, "get restaurant detail Exception: " + t.getMessage());
            }
        });
